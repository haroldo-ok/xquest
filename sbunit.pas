...
interface
uses dos,crt; {****debug only****}
function SbInit(Detect:boolean):word;
procedure SetSBVolume(Volume:integer);
(*procedure SetSBVolume2(Volume:integer);  {DEBUG} *)
procedure SbDone;
(*procedure MixSamples; {debug only} *)
procedure SbPoll;
...
const
  ...
  MaxSoundEffects:integer=4;  {May be varied from within the application,
  ...
  CompSize=255*CompiledMaximumSoundEffects;  {Size of compander table}

  MaxSoundVolume=128;
  SoundVolume:integer=96;
...
Type Soundrec=record
     ...
     CompanderTableType=array[-CompSize div 2..CompSize div 2] of byte;

     DMAPorttype=record
		   Page, Address, Length:Word
		 end;
const
  DMAPort:array[0..7] of DMAPortType=          {DMA port information}
    ((Page: $87; Address: $00; Length: $01),
...
var
  ...
  MixBuffer:array[1..MixBufSize] of word;

  CompanderTable:CompanderTableType;
...
procedure MixSamples;assembler; {Debug}
asm
	mov     si,[DmaBuffer]   {Yes, set up first half}
	push    ds
	push    [DmaBuffer]
...
procedure MixSound;assembler; {need to add in empty samples?}
asm
		push    bp
		push    ax
		...
		mov     ax,bp
		sub     ax,bx
		jle     @@EmptySample  {ax is length of sample remaining}
		cmp     ax,cx
		jg      @@SizeOK
		mov     cx,ax
@@SizeOK:                               {cx now least of cx,ax}
		push    si
		add     bx,dx
		...
		mov     bx,cx
		add     cx,7
		shr     cx,3  {no. repetitions of loop= (cx+7)/8}
		and     bx,7  {count mod 8}
		shl     bx,1  {prepare for word lookup}
		add     si,word ptr cs:[@SiOffsetTable+bx]
		add     di,word ptr cs:[@DiOffsetTable+bx]
		jmp     word ptr cs:[@LoopEntryTable+bx]
@SiOffSetTable:
		dw      0,-7,-6,-5,-4,-3,-2,-1
@DiOffSetTable:
		dw      0,-14,-12,-10,-8,-6,-4,-2

@LoopEntryTable:
		dw      @d8,@d1,@d2,@d3,@d4,@d5,@d6,@d7

@@MixSamp:  {Unrolled loop, 8 times}
@d8:
...
{--------------------------------------------------------------------------
 GetSamples:  Returns the next chunk of samples to be played.
  In:
    ...
    Count   - Byte half-size of DMA Buffer (mix one half while playing the other)
...
@@CopyChunk:    mov     cx,[BufLen]
		cmp     cx,bx              {bx = Count}
		jbe     @@MoveChunk        {is cx<bx? }
		mov     cx,bx              {make cx least of cx, bx}

@@MoveChunk:    mov     si,[BufPtr]
		sub     [BufLen],cx
		sub     bx,cx
		push    bx                 {Count of bytes left to process}
		mov     bx,[si]
		mov     al,byte ptr [CompanderTable+(compsize/2)+bx]
			 {initialise al for loop}
		mov     bx,cx
		add     cx,7
		shr     cx,3  {no. repetitions of loop= (cx+7)/8}
		and     bx,7  {count mod 8}
		...
		add     si,word ptr cs:[@SiOffsetTable+bx]
		add     di,word ptr cs:[@DiOffsetTable+bx]
		jmp     word ptr cs:[@LoopEntryTable2+bx]
@SiOffSetTable:
		dw      0,-14,-12,-10,-8,-6,-4,-2
@DiOffSetTable:
		dw      0,-7,-6,-5,-4,-3,-2,-1
@LoopEntryTable2:
		dw      @q8,@q1,@q2,@q3,@q4,@q5,@q6,@q7
@@CompandLoop:  {***unroll***}
@q8:
		mov     bx,[si+2]
		mov     es:[di],al
		mov     al,byte ptr [CompanderTable+(compsize/2)+bx]
					   {convert mixed word to byte}
...
@q1:
		...
		mov     [BufPtr],si        {Current MixBuffer Position}
		pop     bx
...
{--------------------------------------------------------------------------
; SetSBVolume: Sets up volume table  (max. Volume 127)
...
asm
		...
		mov     dx,CompSize
		shr     dx,1
		neg     dx          {dx= -compsize/2}
		mov     bx,0
		mov     si,Volume
		shl     si,1        {si = volume/128 in 8:8}
		mov     ax,word ptr [Volume]
		cmp     ax,MaxSoundVolume
		jl      @@InBounds
		mov     ax,MaxSoundVolume
@@InBounds:
		imul     dx         {dx:ax = (-compsize/2 * Volume) }
		shl     ax,1
		rcl     dx,1        {dx:ax = (-compsize/2 * Volume)/128 in 24:8}
		add     ax,(127)*256+128   {add 127.5 in 8:8}
		adc     dx,0        {dx:ax = " + 127}
@@Loop:
		add     ax,si
		adc     dx,0
		...
		cmp     dx,0
		jnz     @@TooHigh
		mov     byte ptr CompanderTable[bx],ah
@@OK:
		inc     bx
		loop    @@Loop
		jmp     @@end
@@Negative:
		mov     byte ptr CompanderTable[bx],0
		jmp     @@OK
@@TooHigh:
		mov     byte ptr CompanderTable[bx],255
		jmp     @@OK
@@end:
end;
...
{  ratio:=0;
  repeat
    ratio:=ratio+0.1;
  until ratio*ln(half/ratio+1)>=Volume;
  while (ratio*ln(half/ratio+1))>127 do ratio:=ratio-0.1;
  writeln(ratio);
  readln;
 }
{
  ...
  begin
    j:=(longint(i)*Volume) div 128 + 128;
    if j<0 then j:=0;
    if j>255 then j:=255;
    CompanderTable[i]:=j;
  end;
...
{  if Volume<>0 then
    for i:=-half to 0 do
      CompanderTable[i]:=
	128-round(Volume/128*ratio*ln((-i)/ratio+1))
    else for i:=-half to 0 do CompanderTable[i]:=128;
}

{  for i:=-half to 0 do
    if i<-128 then
      CompanderTable[i]:=0 else CompanderTable[i]:=i+128;
  for i:=0 to half do
    if i>127 then
      CompanderTable[i]:=255 else CompanderTable[i]:=i+128;
}

{  for i:=0 to half do
    CompanderTable[i]:=256-CompanderTable[-i];
}
...
{  writeln(ratio);
  for i:=-half to 0 do
  begin
    writeln(ratio*ln((-i)/ratio+1));
    writeln(CompanderTable[i],' ');
    delay(10);
  end;
}
  writeln;writeln;
